...
namespace CameraDemo
{
    public class AlprSDK
    {
        ...
        private const string dllname = "AlprSDK.dll";
        ...
        [DllImport(dllname, CallingConvention = CallingConvention.StdCall, CharSet = CharSet.Unicode)]
        public static extern int AlprSDK_Trans2Screen(int nHandleID,
            int nScreenType,
            int nFirstLineColor,
            IntPtr pFirstLineData,
            int nSecondLineColor,
            IntPtr pSecondLineData,
            int nThirdLineColor,
            IntPtr pThirdLineData,
            int nFourthLineColor,
            IntPtr pFourthLineData
        );

        [DllImport(dllname, CallingConvention = CallingConvention.StdCall, CharSet = CharSet.Unicode)]
        public static extern int AlprSDK_Trans2Voice(
            int nHandleID,
            int nScreenType,
            byte[] voiceIDList,
            int nCount
        ...
        public delegate void VoiceDataCallbackV2(IntPtr pVoiceData, uint dwDataLen, IntPtr lpUser);

        [DllImport(dllname, CallingConvention = CallingConvention.StdCall, CharSet = CharSet.Unicode)]
        public static extern int AlprSDK_StartVoice(
            int nHandleID,
            uint dwVoiceChan,
            VoiceDataCallbackV2 fpVoiceDataCallBack,
            IntPtr lpUser,
            uint dwAudioType
        );

        public delegate void DevStatusCallback(ref TDevStatus devStatus, ref int len, IntPtr pUserData);

        [DllImport(dllname, CallingConvention = CallingConvention.Winapi)]
        public static extern int AlprSDK_CreateDevStatusCallback(
            int nHandleID,
            DevStatusCallback callback,
            IntPtr pUserData
        );

        [DllImport(dllname, CallingConvention = CallingConvention.StdCall, CharSet = CharSet.Unicode)]
        public static extern int AlprSDK_ClearDevStatusCallback(int nHandleID);

        private const int MAXAUXCAMERANUM = 4;
        private const int MAX_AUX_IN_NUM = 8;

        [StructLayout(LayoutKind.Sequential)]
        public struct AuxCameraStatus
        {
            public int Status;
        }

        [StructLayout(LayoutKind.Sequential)]
        public struct TDevStatus
        {
            [MarshalAs(UnmanagedType.ByValArray, SizeConst = MAXAUXCAMERANUM)]
            public AuxCameraStatus[] auxCameraStatusAarry;

            public int HeartBeatStatus;

            public byte GateSwitchStatus;
            public byte GateRunStatus;
            public byte FeelStatus;
            public byte Reserve1;

            public ushort PhotosensitiveValue;

            [MarshalAs(UnmanagedType.ByValArray, SizeConst = MAX_AUX_IN_NUM)]
            public byte[] AuxInStatus;

            public byte GateOpenStatus;
            public byte GateCloseStatus;
            public byte PrinterStatus;

            public byte TimeoutAlarm;
            public byte ScanCodeStatus;
