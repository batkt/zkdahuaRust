...
impl PlateService {
    ...
    async fn send_once(&self, url: &str, token: &str, payload: &PlatePayload<'_>) -> anyhow::Result<()> {
        ...
        let text = tokio::time::timeout(
            ...
            resp.text(),
        ).await.unwrap_or_else(|_| Ok("<body timeout>".into())).unwrap_or_default();

        if status.is_success() {
            log::info!("PLATE OK | plate={} status={status} body={text}", payload.mashiniiDugaar);
            return Ok(());
        }

        log::error!("PLATE FAIL | plate={} status={status} body={text}", payload.mashiniiDugaar);
