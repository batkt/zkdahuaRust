...
pub async fn run_plate_listener(ip: String, password: String, port: u16, plate_tx: mpsc::Sender<PlateEvent>) {
    ...
    // Reuse client across reconnects — avoids repeated SSL handshakes
    ...
    loop {
        // Only back off on repeated errors, not on clean stream ends
        if error_streak > 0 {
            let delay_secs = std::cmp::min(2u64.pow(error_streak.min(5)), 30);
            info!("[{ip}] Алдааны дараа {delay_secs}s хүлээж байна (#{error_streak})");
            tokio::time::sleep(std::time::Duration::from_secs(delay_secs)).await;
        }

        info!("[{ip}] Plate listener холбогдож байна...");

        match connect_and_read(&client, &ip, &password, &url, &plate_tx).await {
            Ok(_) => {
                // Stream ended cleanly — reconnect immediately
                error_streak = 0;
                info!("[{ip}] Stream дууссан, шууд дахин холбогдоно...");
            }
            Err(e) => {
                error_streak += 1;
                warn!("[{ip}] Холболтын алдаа (#{error_streak}): {e}");
            }
        }
    }
}
...
async fn connect_and_read(
    ...
    info!("[{ip}] Амжилттай холбогдлоо — plate event хүлээж байна...");
                ...
                let key = format!("{ip}:{plate}");
                {
                    ...
                    if let Some(last) = map.get(&key) {
                        if now.duration_since(*last).as_secs() < 10 {
                            info!("[{ip}] Давхардсан plate давсан: {plate}");
                            continue;
                        }
                    }
                    map.insert(key, now);
