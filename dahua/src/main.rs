...
async fn run_app(cfg: Config) -> anyhow::Result<()> {
    ...
    // 2. Init CameraManager — Dahua cameras only (ZK handled by parking-service)
    let mut dahua_cfg = cfg.clone();
    dahua_cfg.cameras.retain(|c| c.camera_type != "zk");
    let manager = CameraManager::new(&dahua_cfg, plate_tx.clone());
    CAMERA_MANAGER.set(manager)
        .map_err(|_| anyhow::anyhow!("CameraManager already initialized"))?;
    ...
    // 5. Start HTTP plate listeners — Dahua cameras only (skip ZK)
    for cam in cfg.cameras.iter().filter(|c| c.camera_type != "zk") {
        let ip       = cam.ip.clone();
    ...
    // 7. API server — Dahua service listens on 5001; ZK service on 5000 forwards here
